    ...
    <view>
			...
			<view class="x12 border-bottom border-gray padding-top padding-bottom" v-for="(item,index) in goodsList" :key="index">
				...
				<view class="tctj-body-right">
					<view class="x-auto" @click="goDetails(item)">
						<view class="tctj-body-right-title">
							<view class="x12" style="overflow:hidden;  text-overflow:ellipsis; white-space:nowrap;">{{item.goods_title}}</view>
							<view class="x12" style="font-size: 10pt; height: 30upx; padding-top: 20upx; color: #999; text-decoration: line-through; font-weight: 100;">
								<view v-if="(item.price*1) < (item.original_price*1)">{{deployinfo.monetary_symbol}}{{item.original_price}}<span style="font-size: 9pt; color: #666;">/{{item.unit}}</span></view>
							</view>
						</view>
						<view class="tctj-body-right-price">
							{{deployinfo.monetary_symbol}}{{item.price}}<span style="font-size: 9pt; color: #666;">/{{item.unit}}</span>
						</view>
					</view>
					<view class="x-auto float-right" style="padding-top: 40upx;">
					...
					<view class="x12" style="font-size: 8pt; color: #fff; margin-top: 4px; padding-bottom: 4px;">
						...
						<view class="float-left" style="border: #FF9900 solid 1px; padding: 1px 4px; border-radius:2px; background-color: #FF9900;" v-if="item.sale_num!='' && item.show_sale_num==1">库存 {{item.sale_num}}</view>
					</view>
				</view>
			</view>
		</view>
		...
		<view class="x12 shop-menu">
			<view class="x12 bg-white text-default text-333 border-top border-gray">
				<view class="x6 text-center" style="line-height: 115upx; width: 60%; font-size: 14pt;" @click="goGwc">
					购物车
				</view>				
				<view class="x6 text-white float-right text-center" style="line-height: 115upx; width: 40%; background-color: #F65A2A; font-size: 14pt;" @click="sm">
					继续扫码
				</view>
			</view>
		</view>
				...
				uni.request({
					...
					success: res => {
							...
							if(res.data.data.goods.length==1){
								this.jrShoppingCart(1,res.data.data.goods[0]);
							}
