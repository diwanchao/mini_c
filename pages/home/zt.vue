<template>
	<view>
		<view class="x12 bg-white">
			<view class="x12" style="height: 272upx;">
				<image :src="img" style="width: 750upx; height: 272upx;"></image>
			</view>
			<view class="x12"  style="width: 750upx;">
				<view class="x4" style="font-size: 10pt; width: 250upx; padding:0 0 2upx 0; background-color: #f5f5f5;" v-for="(g_item,g_index) in goodsList" :key="g_index">
					<view class="x12 bg-white" style="width: 210upx; padding: 30upx 20upx;" v-if="(g_index+1)%3==0">
						<view class="x12" @click="goDetails(g_item)">
							<image :src="g_item.img"  mode="aspectFill" style="width: 209upx; height: 209upx;"></image>
						</view>						
						<view class="x12">
							<view class="x12" style="height: 74upx; overflow:hidden; width: 170upx; text-overflow:ellipsis; font-size: 28upx;"  @click="goDetails(g_item)">
								{{g_item.goods_title}}
							</view>
							<view class="x12">
								<view class="x-auto" style="color: #F65A2A; font-size: 12pt;">
									{{deployinfo.monetary_symbol}}{{g_item.original_price}}
								</view>
							</view>
						</view>
					</view>
					<view class="x12 bg-white" style="width: 209upx; padding: 30upx 20upx;" v-else>
						...
						<view class="x12">
							<view class="x12" style="height: 74upx;overflow:hidden;  text-overflow:ellipsis; font-size: 28upx;" @click="goDetails(g_item)">
								{{g_item.goods_title}} 
							</view>
							<view class="x12">
								<view class="x-auto" style="color: #F65A2A; font-size: 12pt;">
	...
	export default {
		...
		onShareAppMessage(res) {//分享
			try {//从本地缓存中同步获取指定 key 对应的内容。
				...
				return {
				  ...
				  path: '/pages/home/zt?member_openid='+mv.openid+"&stores_id="+xv.store.stores_id+'&group_id='+this.dataZs.group_id+'&title='+this.dataZs.title
				}
			} catch (e) {
				// error
			}
		...
		methods: {
			getHome(data,action = 'add'){
				...
				uni.request({
					url:url.websiteUrl+'/api_v2/mall_shop_v2/group_details',
					method:'POST',
					dataType:'json',
					header:{
